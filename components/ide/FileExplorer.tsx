...
function FileTreeItem({
  ...
  if (node.type === "folder") {
    return (
      <div>
        <div
          className="group flex items-center w-full hover:bg-sidebar-accent cursor-pointer transition-colors"
          style={{ paddingLeft: `${depth * 12 + 8}px` }}
        >
          <button
            onClick={() => setIsOpen(!isOpen)}
            onDoubleClick={() => setRenaming(true)}
            className="cursor-pointer flex items-center gap-1 flex-1 px-2 py-1 text-sm min-w-0"
          >
            {isOpen ? (
  ...
  return (
    <div
      className={`group flex items-center w-full cursor-pointer transition-colors ${
    ...
    >
      <button
        ...
        onDoubleClick={() => setRenaming(true)}
        className="cursor-pointer flex items-center gap-1.5 flex-1 px-2 py-1 text-sm min-w-0"
      >
